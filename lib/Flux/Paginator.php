	...
	public function getHTML()
	{
		...
		for ($i = $start; $i < $end; ++$i) {
			...
			if ($i == $this->currentPage) {
				$pages[] = sprintf(
					'<a title="Página #%d" class="page-num current-page">%d</a>',
					$i, $i
				);
			}
			else {
				$pages[] = sprintf(
					'<a href="%s" title="Página #%d" class="page-num">%d</a>',
					$request, $i, $i
				);
			}
		}
		...
		if ($hasPrev) {
			array_unshift($pages, sprintf('<a href="%s" title="Painel Anterior (p#%d)" class="page-prev">Anterior</a> ', $this->getPageURI($start - 1), $start - 1));
		}
		
		if ($hasNext) {
			array_push($pages, sprintf(' <a href="%s" title="Próximo Painel (p#%d)" class="page-next">Próximo</a>', $this->getPageURI($end), $end));
		}
		...
		if (Flux::config('ShowPageJump') && $this->numberOfPages > Flux::config('PageJumpMinimumPages')) {
			...
			$jump    = '<label>Pular Para Página: <input type="text" name="jump_to_page" id="jump_to_page" size="4" onkeypress="if (event.keyCode == 13) { %s }" /></label>';
