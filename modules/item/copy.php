...
if ($item && count($_POST) && $params->get('copyitem')) {
	...
	if (!$copyID) {
		$errorMessage = 'Você deve especificar o ID do item a ser duplicado.';
	}
	elseif (!ctype_digit($copyID)) {
		$errorMessage = 'ID do item a ser duplicado deve ser um número.';
	}
	else {
		...
		if ($res && $sth->fetch()->itemExists) {
			$errorMessage = 'Um item com essa ID já existe no item_db2.';
		}
		else {
			...
			if ($res) {
				$session->setMessageData("Item tem sido duplicado como #$copyID!");
				...
				$errorMessage = 'Falha ao duplicar item.';
			}
		}
	}
}
